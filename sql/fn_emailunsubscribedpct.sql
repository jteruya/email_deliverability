drop function if exists jt.fn_emailunsubscribedpct();

create function jt.fn_emailunsubscribedpct()
returns table (  metric_desc varchar
        ...
        select cast('unsubscribed:' as varchar) || emailsubjectcatg || ':' || eventtype as metric_desc
             ...
             , case
                 when sum(case when eventstatus = 'delivered' and mailgundate < cast(now() as date) then count else 0 end) > 0 then cast(cast(sum(case when eventstatus = 'unsubscribed' and mailgundate < cast(now() as date) then count else 0 end) as decimal(15,4))/cast(sum(case when eventstatus = 'delivered' and mailgundate < cast(now() as date) then count else 0 end) as decimal(15,4)) as decimal(15,4)) * 100
                 else null
               end as metric_val_0
