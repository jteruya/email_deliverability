...
welcome_delivered_options = {
  chart: {
    renderTo: 'welcome_delivered',
    type: 'spline',
    zoomType: 'x',
    height: 350,
    borderWidth: 1
  ...
  title: {
    text: 'Welcome Email Percentage Delivered'
  },
  tooltip: {
    formatter: function () {
...
$.get('csv/welcomeemaildelivered.csv', function(data) {
  var lines = data.split('\n')
  var linecnt = data.split('\n').length - 1;
  //console.error(linecnt)

  var columns = lines[0].split(',')
  //welcome_delivered_options.xAxis.title.text = columns[0]
  $.each(lines, function(lineNo, line) {
    if (lineNo == linecnt) {
      ...
      welcome_delivered_options.series.push(series)
    }
  })

welcome_delivered = new Highcharts.Chart(welcome_delivered_options);
