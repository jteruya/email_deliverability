...
dailydigest_delivered_options = {
  chart: {
    renderTo: 'dailydigest_delivered',
    type: 'spline',
    zoomType: 'x',
    height: 350,
    borderWidth: 1
  ...
  title: {
    text: 'Daily Digest Percentage Delivered'
  },
  tooltip: {
    formatter: function () {
...
$.get('csv/dailydigestemaildelivered.csv', function(data) {
  var lines = data.split('\n')
  var linecnt = data.split('\n').length - 1;
  //console.error(linecnt)

  var columns = lines[0].split(',')
  //dailydigest_delivered_options.xAxis.title.text = columns[0]
  $.each(lines, function(lineNo, line) {
    if (lineNo == linecnt) {
      ...
      dailydigest_delivered_options.series.push(series)
    }
  })

dailydigest_delivered = new Highcharts.Chart(dailydigest_delivered_options);
